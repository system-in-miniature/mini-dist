#!/usr/bin/env python3
"""Build, verify, and prepare MiniDist Journey workspaces."""
...
def patch_files(stage: Stage) -> tuple[str, ...]:
    paths = tuple(match.group(2) for match in PATCH_HEADER.finditer(stage.patch.read_text()))
...
def check_chain(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="minidist-journey-chain-") as raw:
        workspace = Path(raw)
...
def assert_stage_parity(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f"minidist-stage-{stage_number:02d}-") as raw:
        expected = Path(raw)
...
def prepare_workspace(
    ...
    *,
    ...
    forbidden = {Path("/"), Path.home().resolve(), ROOT.resolve(), ROOT.parent.resolve()}
...
def main() -> int:
    ...
    default = ROOT.parent / "MiniDist-journey-workspace"
