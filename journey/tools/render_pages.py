...
REPO_URL = "https://github.com/system-in-miniature/mini-dist"
META = re.compile(
...
def role_rank(path: str) -> int:
    if path.startswith("src/minidist/sim/"):
        return 0
    if path.startswith("src/minidist/protocols/"):
        return 1
    ...
    *,
    ...
    while index < len(markers):
        ...
        if not file_body.startswith("#### "):
            raise ValueError(f"{label} {paths}: marker group requires one #### explanation")
        for path in paths:
        ...
        for code in slices:
            ...
            if code and not any(
                ...
                raise ValueError(f"{label} {paths}: key code does not match stage.patch")
        files.extend(
...
def render_index(cards: list[Card], *, chinese: bool) -> str:
    if chinese:
        ...
        lines = [
            ...
            "",
            "This is the browser-based path among MiniDist's three learning modes. Use the [Mechanism Tutorial](../index.md) for topic-oriented study, or the [Agent-Guided usage guide](../agent-guided.md) for interactive CLI teaching.",
            "",
            "For an editor-focused diff, run `python -m journey.tools.build_journey study N` and open `../MiniDist-journey-workspace`.",
            "",
