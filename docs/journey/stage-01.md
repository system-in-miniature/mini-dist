# Stage 01 · Deterministic simulation substrate
...
Make time, tie-breaking, and traces explicit so one experiment script always produces the same event history.

??? note "File diff: src/minidist/sim/clock.py"
    ```diff
...
??? note "File diff: src/minidist/sim/trace.py"
    ```diff
...
??? note "File diff: src/minidist/sim/scheduler.py"
    ```diff
...
Model delay, drop, reorder, partition, volatile loss, and restart as reproducible state transitions.

??? note "File diff: src/minidist/sim/network.py"
    ```diff
...
Install the package, export the simulator vocabulary, and pin the test environment without treating project wiring as a distributed protocol.

??? note "Supporting file diffs (5 files)"
    **`pyproject.toml`**
...
[Compare this stage on GitHub](https://github.com/system-in-miniature/mini-dist/tree/stage-01)
