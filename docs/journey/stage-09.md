# Stage 09 · ISR membership and high watermark
...
??? note "File diff: tests/protocols/test_isr.py"
    ```diff
...
Connect dynamic in-sync membership to availability, committed visibility, clean election, catch-up, and lease-based authority.

??? note "File diff: src/minidist/protocols/isr/group.py"
    ```diff
...
Expose the group, log entry, catch-up mode, and probe values as supporting wiring.
...
[Compare this stage on GitHub](https://github.com/system-in-miniature/mini-dist/compare/stage-08...stage-09)
