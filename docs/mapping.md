...
offset/backlog-based resynchronization, full replacement, and manual primary
change.
...
| MiniDist Raft mechanism | Paper section | Correspondence and experiment boundary |
|---|---|---|
| follower / candidate / leader and monotonically increasing terms | §5, §5.1 | On receiving a higher-term RequestVote, AppendEntries, or response, the node updates its persistent term and steps down; this fences the old leader in experiment 3. |
| Randomized election timeout, candidate self-vote, and majority election through RequestVote | §5.2, §5.6 | The timeout is drawn from the private random source associated with the constructor seed; neither wall-clock time nor module-level random state is read. |
| A candidate's log must be at least as up-to-date as the voter's log | §5.4.1 | The `(lastLogTerm, lastLogIndex)` pair is compared lexicographically, preventing election of a leader that lacks committed log entries. |
| AppendEntries heartbeats and log replication | §5.2, §5.3 | Every RPC goes through `SimNet`; a heartbeat is AppendEntries with empty entries. |
| `prevLogIndex` / `prevLogTerm` consistency checks, conflicting-suffix deletion, and `nextIndex` fallback and retry | §5.3, Figure 2 | The follower returns the start of the conflicting term, and the leader retransmits from that point. When experiment 3 heals, this overwrites the uncommitted fork of the old minority leader. |
| Majority replication advances commitIndex | §5.3 | An entry can be committed only when `matchIndex` covers a majority; QUORUM is the only supported write ack. |
| Advance commitIndex by replica count only for “entries from the current term” | §5.4.2 | The code's why-comment directly cites the counterexample in this section: an old-term entry cannot be committed directly merely because it is currently stored on a majority; it is committed indirectly as part of the committed prefix of a current-term entry. |
| The leader appends a no-op after election | §8 | The no-op provides a committable point in the current term and safely commits earlier prefixes after recovery or a primary change; it does not alter the KV state. |
| `currentTerm`, `votedFor`, and the log are persistent; other state is volatile | §5 / Figure 2 | A crash preserves these three items; restart rebuilds the node as a follower with commitIndex=0 and an empty state machine, then replays the committed prefix from leaderCommit. |
| Direct `LEADER` read | Client-interaction background in §8 | This only routes to the currently known leader with the highest term and does not separately prove that the leader still controls a majority, so it makes no linearizability claim. |
| Simplified read-index barrier for `LINEARIZABLE` reads | Read-only request rules in §8 | The leader sends empty AppendEntries in the current term and reads the applied state machine after receiving successful responses from a majority; the read command is not written to the log. |
